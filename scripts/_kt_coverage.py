...
if __name__ == "__main__":
    ...
    print("  17 of 30 reducible directions.")
    print()
    print("  *** THE sGB ROW ABOVE IS WRONG AND IS KEPT AS THE COUNTEREXAMPLE. *** It says 100% at")
    print("  every rank because this model only asks whether the BACKGROUND product p_t^a p_phi^b")
    print("  H^c fits (c+e <= denpow). What the solver actually has to represent is that product's")
    print("  ZETA-CORRECTION, which carries EXTRA denominator powers the background product does")
    print("  not. At rank 4 the c=2 direction needs 2(HK0 H1_2 + HK1 H1_1 + HK2 H1_0), whose")
    print("  denominator does not divide L^6 -- so the true sGB coverage at rank 4 / denpow 6 is")
    print("  8 of 9, not 9 of 9. Measured, not modelled: see §132.")
    print()
    print("  So the fix for an unstated coverage assumption was itself published with an unstated")
    print("  coverage assumption, within a day. The lesson is not 'model harder' -- it is that")
    print("  coverage must be MEASURED by asking the representability guard about the object the")
    print("  solver actually needs, which is what _kt_double.py now does at run time. A combinatorial")
    print("  model of a search space is one more thing that can be wrong about it.")
