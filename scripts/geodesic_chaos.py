...
def _rhs(g_func, s):
    x, u = s[:4], s[4:]
    G = christoffel_numeric(g_func, x)
...
def _rk4(g_func, s, h):
    k1 = _rhs(g_func, s)
    k2 = _rhs(g_func, [s[i] + h / 2 * k1[i] for i in range(8)])
    k3 = _rhs(g_func, [s[i] + h / 2 * k2[i] for i in range(8)])
    k4 = _rhs(g_func, [s[i] + h * k3[i] for i in range(8)])
    return [s[i] + h / 6 * (k1[i] + 2 * k2[i] + 2 * k3[i] + k4[i]) for i in range(8)]


def trajectory(g_func, x0, u0, dtau=0.1, steps=200):
    """RK4-integrate the geodesic ẍ^a = −Γ^a_bc ẋ^b ẋ^c. Returns the list of
    phase-space states [x(4), u(4)] along the path."""
    s = list(x0) + list(u0)
    out = [s[:]]
    for _ in range(steps):
        s = _rk4(g_func, s, dtau)
        out.append(s[:])
    return out


def lyapunov(g_func, x0, u0, dtau=0.15, blocks=500, renorm_every=4, d0=1e-8, idx=5):
