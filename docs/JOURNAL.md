...
## 2026-09-23 (night) — a counterexample to my own brief, found in our own workspace; and the rank-8 knob

**The pp-wave (RESULTS §147).** My moonshot #1, and CLAUDE.md §1, said no Lorentzian vacuum spacetime
with an irreducible rank-≥3 Killing tensor is known. This project's `high_rank_killing` session folder
had built one on 2026-09-05. Verified here from the object alone, with a standalone script that imports
nothing from the repo, before reading the source: vacuum, (1,3), {H,F} = 0, dim K1 = 2 by integrability
conditions (first-order bound 6, not tight; 2 from k = 1; Schwarzschild control 4), polynomially
irreducible by the ideal argument. Functionally it is the bracket of two quadratic integrals — a
superintegrable system — which the source had disclosed; my first encoding of that disclosure failed
because I hadn't said *dependent on what*. Brought in as `workspaces/high_rank_killing/` with a provenance
file; the source folder untouched; copyrighted PDFs and the sisters' plans deliberately left behind.
CLAUDE.md §1 and both plan documents corrected; the moonshot survives in a sharper form (a harmonic
potential whose higher integral is NOT generated by quadratics, or rank 4).

**Rank 8, axial.** The margin-6 rerun also kept random at 24 < 25, so "margin too narrow" was wrong: I had
changed the knob that differed from §143's reference rather than the one controlling the failure. A
minutes-long diagnostic then measured it: the H₀⁴ floor extension needs L⁸ under `o3rphi`/`o3yphi` (L⁷
