...
field `dA ∧ dt` sources exactly the Kepler term of SW-IV. **Verified** (`scripts/exp004_kk_reduction.py`):
the constant-dilaton KK identities `R̂_μν = R⁽⁴⁾_μν − ½F_μρF_ν^ρ`, `R̂_μv = ½∇^ρF_μρ`, `R̂_vv = ¼F²`
hold identically on this family with `U, A` arbitrary functions of `(ξ,η)`; on the solution
`∇_ρF^ρμ = 0` (`A_t = 2κ Re z^{−1/2}` is harmonic), `F_ρσF^ρσ = 0` (null, so the constant dilaton is
consistent), and `R⁽⁴⁾_μν = ½F_μρF_ν^ρ`, whose only component is `R⁽⁴⁾_tt = κ²/(4ρ⁶) ≥ 0`. In
Gaussian units (`G_μν = 8πT_μν`) the physical potential is `A/2`. Controls: the wrong coupling
`R⁽⁴⁾ = F F` and a non-harmonic `A` both fail.

A consequence, also verified (`scripts/exp004_4d_em_kt.py`): since `H₅|_{p_v=0} = H₄`, the cubic at
`p_v = 0` is a rank-3 Killing tensor **of the 4D Einstein–Maxwell metric itself** —
`{H₄, F} = 0` exactly, exactly two Killing vectors (jet count), pure part nonzero → polynomially
irreducible, functionally dependent. It is not new in kind: it is the Eisenhart lift of SW-IV with
`σ ≠ 0`, i.e. the non-vacuum (null-dust/null-Maxwell) sector that the literature's constructions
already occupy. What the 5D metric adds is that this sector uplifts to **vacuum** one dimension up.
