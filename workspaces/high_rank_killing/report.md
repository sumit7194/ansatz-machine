...
SW-IV (standard reduction; verified in EXP-004). Grade: **verified**; the count is ansatz-scoped as in EXP-002.
...
## EXP-004  Kaluza–Klein reduction of the 5D vacuum metric — 2026-09-23

**Goal.** Verify the reading stated in EXP-003: `ds₅² = ds₄² + (dv + A dt)²` with `ds₄²` a 4D
Einstein–Maxwell pp-wave whose null Maxwell field sources the Kepler term of SW-IV.

**Hypothesis.** Constant-dilaton KK: `R̂_μν = R⁽⁴⁾_μν − ½F_μρF_ν^ρ`, `R̂_μv = ½∇^ρF_μρ`,
`R̂_vv = ¼F²` (horizontal frame `e^v = dv + A`). 5D Ricci-flat ⇔ 4D Einstein–Maxwell in KK
normalisation + Maxwell + `F² = 0`.

**Method (M2).** `A = A_μdx^μ = A dt`, `A = 2κξ/ρ²`, `F = dA`; 4D metric `g₄` with `2V = 2U + A²`;
`R⁽⁴⁾`, `∇`, raised indices all of `g₄`. The identities are **not assumed**: step B computes the 5D
Ricci tensor for `U(ξ,η)`, `A(ξ,η)` arbitrary functions and compares all 15 coordinate components
with the frame prediction mapped back (`R⁵_μν = R̂_μν + A_μR̂_vν + A_νR̂_μv + A_μA_νR̂_vv`, etc.).
Step C evaluates on the solution with `a, κ` symbolic. Step D: known-fail controls.

**Implementation.** `scripts/exp004_kk_reduction.py`, `scripts/exp004_4d_em_kt.py`;
`results/exp004_kk.log`, `results/exp004_kk_firstrun.log` (kept), `results/exp004_4d_em_kt.log`.
Runtime ~1 s each; nothing launched in the background; sibling repo untouched.

**Results.**

| check | result |
|---|---|
| `ds₅² − ds₄² − (dv + A dt)² = 0`; `2V = (4aξ + κ²)/ρ²` | identity, exact |
| KK identities off-shell (arbitrary `U, A`), 15 components | **hold** (after the sign fix below) |
| 5D Ricci on the solution | `≡ 0` |
| Maxwell `∇_ρF^ρμ = 0` | holds (`A_t` harmonic: `Δ(2κξ/ρ²) = 0`) |
| `F_ρσF^ρσ = 0` | holds (null field; constant dilaton consistent) |
| `R⁽⁴⁾_μν = ½F_μρF_ν^ρ` | holds; only component `R⁽⁴⁾_tt = κ²/(4ρ⁶) ≥ 0` |
| source location | `Δ(4aξ/ρ²) = 0` (vacuum part), `Δ(κ²/ρ²) = κ²/(2ρ⁶)` (Kepler part, `κ²/ρ² = κ²/r`) |
| κ → 0 | `R⁽⁴⁾ ≡ 0`: the EXP-002 vacuum pp-wave |
| known-fail: `R⁽⁴⁾ = F F` | fails (as it must) |
| known-fail: non-harmonic `A = 2κξ²/ρ²` | Maxwell fails; KK identity still holds off-shell (so the identity check is not tied to the solution) |
| 4D base itself | `F|_{p_v=0}` is a rank-3 KT of `g₄`: `{H₄,F} = 0` exact; `dim K1 = 2` (jets, two points); pure part ≠ 0 → polynomially irreducible; `rank(p_t,p_s,H,Q1,Q2) = 5 = rank(…,F)` → functionally dependent; matches the WRITEUP §6 formula |

**Two checks that fired, both on the check.** (1) Step B's first run reported a mismatch in
`(t,v)` and `(t,t)` equal to exactly `−ΔA/(8ρ²)` = −2 × prediction: I had written
`R̂_μv = +½∇^ρF_ρμ`; the correct sign is `½∇^ρF_μρ`. The off-shell test is what caught it — an
on-shell-only test would have passed silently, since `ΔA = 0` on the solution. (2) A positivity
line compared against a hard-coded `κ²/(16ρ⁶)`; the computed value is `κ²/(4ρ⁶)`. Both kept in
`results/exp004_kk_firstrun.log`.

**Analysis.** The KK reading is exact: the 5D Lorentzian vacuum metric of EXP-003 is the lift of a
4D Einstein–Maxwell pp-wave with a null Maxwell field (physical potential `A/2` in Gaussian units)
whose stress tensor sources precisely the `κ²/r` Kepler term. The 4D base carries the same rank-3
Killing tensor, but it is a non-vacuum Eisenhart lift (SW-IV with σ ≠ 0) of the kind already in the
literature, so it adds nothing to the vacuum claim; what the 5D metric adds is that this sector
uplifts to vacuum.

**Grade.** Verified (exact symbolic, `a, κ` symbolic; KK identities checked on the family, not quoted).

**Next steps.** None; the last open item in `TODO.md` is closed.
